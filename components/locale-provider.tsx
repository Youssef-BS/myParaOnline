...
const translations = {
  en: {
    ...
    curated: 'Curated', essentials: 'Health essentials', add: 'Add', adding: 'Adding', removeWishlist: 'Remove from wishlist', addWishlist: 'Add to wishlist',
    ...
    freeDelivery: 'Free delivery on orders over 150 د.ت · Thoughtful care, delivered', fastDelivery: 'Fast, careful delivery', fromDoor: 'From our door to yours', quality: 'Quality you can trust', selected: 'Selected with intention', guidance: 'Guidance when you need it', support: 'Simple, human support', formula: 'Thoughtful formulas, never overwhelming.', trustedBrands: 'Trusted brands, clearly explained.', joinHealthHub: 'Join HealthHub for considered products, helpful guidance, and a little more confidence in your routine.',
  },
  fr: {
    shop: 'Boutique', categories: 'Catégories', approach: 'Notre approche', cart: 'Panier', language: 'Langue',
    curated: 'Sélection', essentials: 'Essentiels santé', add: 'Ajouter', adding: 'Ajout', removeWishlist: 'Retirer des favoris', addWishlist: 'Ajouter aux favoris',
  ...
  ar: {
    ...
    freeDelivery: 'توصيل مجاني للطلبات فوق 150 د.ت · عناية تصل إليك', fastDelivery: 'توصيل سريع وآمن', fromDoor: 'من بابنا إلى بابك', quality: 'جودة يمكنك الوثوق بها', selected: 'مختارة بعناية', guidance: 'نصائح عند الحاجة', support: 'دعم بسيط وإنساني', formula: 'تركيبات مدروسة بعناية، دون مبالغة.', trustedBrands: 'علامات موثوقة، مشروحة بوضوح.', joinHealthHub: 'انضم إلى HealthHub لاكتشاف منتجات مختارة ونصائح مفيدة وثقة أكبر في روتينك.',
  },
} as const
...
export function LocaleProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const stored = window.localStorage.getItem('healthhub-locale') as Locale | null
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem('healthhub-locale', locale)
  }, [locale])
